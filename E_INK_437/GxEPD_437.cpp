// class GxEPD_437 : Display class for GDEW0154Z17 e-Paper from Dalian Good Display Co., Ltd.: www.good-display.com
//
...
#define GxEPD_437_PU_DELAY 500

GxEPD_437::GxEPD_437(GxIO& io, int8_t rst, int8_t busy)
  : GxEPD(EPD_WIDTH, EPD_HEIGHT), IO(io),
...
void GxEPD_437::drawPixel(int16_t x, int16_t y, uint16_t color)
{
  ...
  {
    y -= _current_page * GxEPD_437_PAGE_HEIGHT;
    if ((y < 0) || (y >= GxEPD_437_PAGE_HEIGHT)) return;
    i = x / 8 + y * EPD_WIDTH / 8;
  }
...
void GxEPD_437::update(void)
{
  if (_current_page != -1) return;
  _using_partial_mode = false;
  _wakeUp();
  _writeCommand(0x10);
  for (uint32_t i = 0; i < GxEPD_437_BUFFER_SIZE; i++)
  {
...
void  GxEPD_437::drawBitmap(const uint8_t *bitmap, uint16_t x, uint16_t y, uint16_t w, uint16_t h, uint16_t color, int16_t mode)
{
  if (mode & bm_default) mode |= bm_invert;
  drawBitmapBM(bitmap, x, y, w, h, color, mode);
}

void GxEPD_437::drawExamplePicture(const uint8_t* black_bitmap, const uint8_t* red_bitmap, uint32_t black_size, uint32_t red_size)
{
  drawPicture(black_bitmap, red_bitmap, black_size, red_size, bm_invert_red);
}

void GxEPD_437::drawPicture(const uint8_t* black_bitmap, const uint8_t* red_bitmap, uint32_t black_size, uint32_t red_size, int16_t mode)
{
  if (_current_page != -1) return;
  _using_partial_mode = false;
  _wakeUp();
...
void GxEPD_437::drawBitmap(const uint8_t* bitmap, uint32_t size, int16_t mode)
{
  if (_current_page != -1) return;
  ...
  if (mode & bm_partial_update)
  {
    ...
    _setPartialRamArea(0, 0, EPD_WIDTH - 1, EPD_HEIGHT - 1);
    _writeCommand(0x10);
    for (uint32_t i = 0; i < GxEPD_437_BUFFER_SIZE; i++)
    {
      uint8_t data = 0xFF; // white is 0xFF on device
...
void GxEPD_437::eraseDisplay(bool using_partial_update)
{
  if (_current_page != -1) return;
  if (using_partial_update)
  {
    ...
    _setPartialRamArea(0, 0, EPD_WIDTH - 1, EPD_HEIGHT - 1);
    _writeCommand(0x10);
    for (uint32_t i = 0; i < GxEPD_437_BUFFER_SIZE * 2; i++)
    {
      _writeData(0xFF); // white is 0xFF on device
    }
    _writeCommand(0x13);
...
void GxEPD_437::updateWindow(uint16_t x, uint16_t y, uint16_t w, uint16_t h, bool using_rotation)
{
  if (_current_page != -1) return;
  ...
  uint16_t xe = gx_uint16_min(EPD_WIDTH, x + w) - 1;
  uint16_t ye = gx_uint16_min(EPD_HEIGHT, y + h) - 1;
  ...
  for (int16_t y1 = y; y1 <= ye; y1++)
  {
    ...
    {
      uint16_t idx = y1 * (EPD_WIDTH / 8) + x1;
  ...
  IO.writeCommandTransaction(0x92); // partial out
  IO.writeCommandTransaction(0x92); // partial out
  delay(GxEPD_437_PU_DELAY); // don't stress this display
}

void GxEPD_437::updateToWindow(uint16_t xs, uint16_t ys, uint16_t xd, uint16_t yd, uint16_t w, uint16_t h, bool using_rotation)
{
  if (!_using_partial_mode) _wakeUp();
  _using_partial_mode = true;
...
void GxEPD_437::_writeToWindow(uint16_t xs, uint16_t ys, uint16_t xd, uint16_t yd, uint16_t w, uint16_t h, bool using_rotation)
{
  if (using_rotation)
  {
    switch (getRotation())
  ...
  if (xs >= EPD_WIDTH) return;
  if (ys >= EPD_HEIGHT) return;
  if (xd >= EPD_WIDTH) return;
  if (yd >= EPD_HEIGHT) return;
  // the screen limits are the hard limits
  uint16_t xde = gx_uint16_min(EPD_WIDTH, xd + w) - 1;
  uint16_t yde = gx_uint16_min(EPD_HEIGHT, yd + h) - 1;
  IO.writeCommandTransaction(0x91); // partial in
...
void GxEPD_437::powerDown()
{
  _using_partial_mode = false; // force _wakeUp()
  _sleep();
}

uint16_t GxEPD_437::_setPartialRamArea(uint16_t x, uint16_t y, uint16_t xe, uint16_t ye)
{
...
void GxEPD_437::_writeCommand(uint8_t command)
{
  IO.writeCommandTransaction(command);
}

void GxEPD_437::_writeData(uint8_t data)
{
  IO.writeDataTransaction(data);
}

void GxEPD_437::_waitWhileBusy(const char* comment)
{
...
void GxEPD_437::drawPaged(void (*drawCallback)(void))
{
  if (_current_page != -1) return;
  _using_partial_mode = false;
  _wakeUp();
  _writeCommand(0x10);
  for (_current_page = 0; _current_page < GxEPD_437_PAGES; _current_page++)
  {
    fillScreen(GxEPD_WHITE);
    drawCallback();
    for (int16_t y1 = 0; y1 < GxEPD_437_PAGE_HEIGHT; y1++)
    {
      for (int16_t x1 = 0; x1 < EPD_WIDTH / 8; x1++)
      {
        uint16_t idx = y1 * (EPD_WIDTH / 8) + x1;
...
void GxEPD_437::drawPaged(void (*drawCallback)(uint32_t), uint32_t p)
{
  if (_current_page != -1) return;
  _using_partial_mode = false;
  _wakeUp();
...
void GxEPD_437::drawPaged(void (*drawCallback)(const void*), const void* p)
{
  if (_current_page != -1) return;
  _using_partial_mode = false;
  _wakeUp();
...
void GxEPD_437::drawPaged(void (*drawCallback)(const void*, const void*), const void* p1, const void* p2)
{
  if (_current_page != -1) return;
  _using_partial_mode = false;
  _wakeUp();
...
void GxEPD_437::_rotate(uint16_t& x, uint16_t& y, uint16_t& w, uint16_t& h)
{
  switch (getRotation())
  {
    case 1:
...
void GxEPD_437::drawPagedToWindow(void (*drawCallback)(void), uint16_t x, uint16_t y, uint16_t w, uint16_t h)
{
  if (_current_page != -1) return;
  _rotate(x, y, w, h);
  ...
  for (_current_page = 0; _current_page < GxEPD_437_PAGES; _current_page++)
  {
    uint16_t yds = gx_uint16_max(y, _current_page * GxEPD_437_PAGE_HEIGHT);
    uint16_t yde = gx_uint16_min(y + h, (_current_page + 1) * GxEPD_437_PAGE_HEIGHT);
    if (yde > yds)
    {
      fillScreen(GxEPD_WHITE);
      drawCallback();
      uint16_t ys = yds % GxEPD_437_PAGE_HEIGHT;
...
void GxEPD_437::drawPagedToWindow(void (*drawCallback)(uint32_t), uint16_t x, uint16_t y, uint16_t w, uint16_t h, uint32_t p)
{
  if (_current_page != -1) return;
  _rotate(x, y, w, h);
  if (!_using_partial_mode)
...
void GxEPD_437::drawPagedToWindow(void (*drawCallback)(const void*), uint16_t x, uint16_t y, uint16_t w, uint16_t h, const void* p)
{
  if (_current_page != -1) return;
  _rotate(x, y, w, h);
  if (!_using_partial_mode)
...
void GxEPD_437::drawPagedToWindow(void (*drawCallback)(const void*, const void*), uint16_t x, uint16_t y, uint16_t w, uint16_t h, const void* p1, const void* p2)
{
  if (_current_page != -1) return;
  _rotate(x, y, w, h);
  if (!_using_partial_mode)
...
void GxEPD_437::drawCornerTest(uint8_t em)
{
  if (_current_page != -1) return;
  _using_partial_mode = false;
  _wakeUp();
  ...
  {
    for (uint32_t x = 0; x < EPD_WIDTH / 8; x++)
    {
      uint8_t data = 0xFF; // white is 0xFF on device
      if ((x < 1) && (y < 8)) data = 0x00;
      if ((x > EPD_WIDTH / 8 - 3) && (y < 16)) data = 0x00;
      if ((x > EPD_WIDTH / 8 - 4) && (y > EPD_HEIGHT - 25)) data = 0x00;
      if ((x < 4) && (y > EPD_HEIGHT - 33)) data = 0x00;
      _writeData(data);
    }
  }
  _writeCommand(0x13);
