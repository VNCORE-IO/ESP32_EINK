...
// https://www.pervasivedisplays.com/products/4-37-e-ink-displays/
...
#define GxEPD_437_BUFFER_SIZE (uint32_t(EPD_WIDTH) * uint32_t(EPD_HEIGHT) / 8)

// divisor for AVR, should be factor of EPD_HEIGHT
#define GxEPD_437_PAGES 8

#define GxEPD_437_PAGE_HEIGHT (EPD_HEIGHT / GxEPD_437_PAGES)
#define GxEPD_437_PAGE_SIZE (GxEPD_437_BUFFER_SIZE / GxEPD_437_PAGES)

class GxEPD_437 : public GxEPD
{
  public:
#if defined(ESP8266)
    //GxEPD_437(GxIO& io, int8_t rst = D4, int8_t busy = D2);
    // use pin numbers, other ESP8266 than Wemos may not use Dx names
    GxEPD_437(GxIO& io, int8_t rst = 2, int8_t busy = 4);
#else
    GxEPD_437(GxIO& io, int8_t rst = 9, int8_t busy = 7);
#endif
    ...
    void powerDown();
    // paged drawing, for limited RAM, drawCallback() is called GxEPD_437_PAGES times
  ...
  private:
#if defined(__AVR)
    uint8_t _black_buffer[GxEPD_437_PAGE_SIZE];
    uint8_t _red_buffer[GxEPD_437_PAGE_SIZE];
#else
    uint8_t _black_buffer[GxEPD_437_BUFFER_SIZE];
    uint8_t _red_buffer[GxEPD_437_BUFFER_SIZE];
#endif
    GxIO& IO;
...
#ifndef GxEPD_Class
#define GxEPD_Class GxEPD_437
#define GxEPD_WIDTH EPD_WIDTH
#define GxEPD_HEIGHT EPD_HEIGHT
#define GxEPD_BitmapExamples <GxEPD_437/BitmapExamples.h>
#define GxEPD_BitmapExamplesQ "GxEPD_437/BitmapExamples.h"
#endif
