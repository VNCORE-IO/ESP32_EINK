...
GxEPD2_437::GxEPD2_437(int8_t cs, int8_t dc, int8_t rst, int8_t busy)
  : Adafruit_GFX(WIDTH, HEIGHT), _cs(cs), _dc(dc), _rst(rst), _busy(busy)
{
}

void GxEPD2_437::_writeCmd(uint8_t c)
{
...
void GxEPD2_437::_writeData(uint8_t d)
{
  digitalWrite(_cs, LOW);
...
void GxEPD2_437::init(uint32_t serial_diag_bitrate)
{
...
void GxEPD2_437::_initDisplay()
{
  ...
  // NOTE: panel-setting data bytes below are carried over from the 152x296
  // Z39c driver as a starting point (LUT from OTP / VCOM-to-0V-fast).
  // VERIFY against your panel's datasheet — this byte also encodes the
  // resolution-select and scan-direction bits on some controllers.
  _writeCmd(0x00);      // panel setting
  _writeData(0x0f);     // LUT from OTP
  _writeData(0x0d);     // VCOM to 0V fast

  _writeCmd(0x61);      // resolution setting
  _writeData(0xB0);     // HRES = 176 (0xB0)
  _writeData(0x01);     // VRES high byte
  _writeData(0xE0);     // VRES low byte -> 0x01E0 = 480

  // NOTE: VCOM/data-interval byte below also carried over unverified —
  // check your panel's recommended VBD (border) setting.
  _writeCmd(0x50);      // VCOM and data interval setting
  _writeData(0x77);     // WBmode: VBDF 17 | D7 VBDW 97 VBDB 57
}

void GxEPD2_437::fillScreen(uint16_t color)
{
...
void GxEPD2_437::drawPixel(int16_t x, int16_t y, uint16_t color)
{
...
void GxEPD2_437::drawBitmap(int16_t x, int16_t y, const uint8_t bitmap[], int16_t w, int16_t h, uint16_t color)
{
...
void GxEPD2_437::writeFullScreenBitmap(const uint8_t* blackBitmap, const uint8_t* redBitmap)
{
