...
void EPD_DRAW_BITMAP(uint16_t x, uint16_t y, uint16_t iw, uint16_t ih, const unsigned char* dataIMG, uint8_t color) {
	// unsigned int i,j, _x, _y, z = 0;
	// uint8_t byte=0;
	// uint16_t byteWidth=(iw + 7) / 8;
	// if (color == 0) {
	// 	EPD_W21_WriteCMD(0x24);  //Black
	// 	for (_y = 0; _y < EPD_HEIGHT; _y++) {
	// 		for (_x = 0; _x < EPD_WIDTH; _x++) {
	// 			if (_x >= x && _x < x + iw && _y >= y && _y < y + ih) {
	// 				//Serial.println(String(_x) + "/" + String(_y));
	// 				i=_x -x;j=_y-y;
	// 				if (i & 7) byte <<= 1;
	// 				else byte = pgm_read_byte(&dataIMG[j * byteWidth + i / 8]);
	// 				if (byte & 0x80) EPD_W21_WriteDATA(0xFF);
	...
	// 	}
	// } else {
	// 	EPD_W21_WriteCMD(0x26);  //Red
	// 	for (_y = 0; _y < EPD_HEIGHT/2; _y++) {
	// 		for (_x = 0; _x < EPD_WIDTH/4; _x++) {
	// 			if (_x >= x && _x < x + iw && _y >= y && _y < y + ih) {
	// 				//Serial.println(String(_x) + "/" + String(_y));
	// 				EPD_W21_WriteDATA(~dataIMG[z]);
	// 				z++;
	// 			} else {
	// 				EPD_W21_WriteDATA(~0xFF);
	// 			}
